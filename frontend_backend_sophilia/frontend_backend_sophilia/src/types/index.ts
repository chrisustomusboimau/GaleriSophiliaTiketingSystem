...
export interface TransactionEntry {
  ...
  /**
   * Verifikasi Checker — LEGACY, dipertahankan untuk data lama. TIDAK
   * LAGI dipakai untuk mengunci hak edit/hapus (lihat `status` +
   * `confirmed_by_*` di bawah) sejak role Checker menjadi read-only.
   */
  ...
  /**
   * Staf (kasir/admin) yang menekan Konfirmasi — mengunci tiket ke
   * status "confirmed"/"paid". `confirmed_by_email` dipakai
   * `getStaffNameFromEmail()` untuk kolom "Dikonfirmasi Oleh".
   */
