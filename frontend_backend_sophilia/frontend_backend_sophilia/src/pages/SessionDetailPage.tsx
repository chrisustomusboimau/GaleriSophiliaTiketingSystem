 ...
 * `GET /transactions/all?session_id=<id>` (difilter di backend).
 *
 * Ekspor Excel per sesi (`GET /reports/sessions/{id}`) khusus ADMIN —
 * tombolnya disembunyikan untuk role lain, dan backend membalas 403.
 *
...
const SessionDetailContent: React.FC<{ sessionId: string }> = ({ sessionId }) => {
  ...
  const exportSlot =
    user.role === "admin" && session ? (
      <ReportExportButton reportPath={`/reports/sessions/${session.id}`} label="Download Excel Per Sesi" />
    ) : null;
