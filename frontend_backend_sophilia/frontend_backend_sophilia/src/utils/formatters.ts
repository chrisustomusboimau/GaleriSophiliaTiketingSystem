...
/**
 * Menurunkan nama tampilan staf dari email login-nya, dengan memotong
 * segala sesuatu MULAI dari karakter "@" (mis. "staff.cashier@domain.com"
 * -> "staff.cashier"). Dipakai kolom "Dikonfirmasi Oleh" di tabel Riwayat
 * Transaksi & Antrian Kasir — MENGGANTIKAN kolom "Verifikasi" (Checker)
 * yang sudah dihapus sejak role Checker menjadi read-only.
 *
 * Sengaja tidak melempar error untuk input kosong/aneh: transaksi yang
 * belum dikonfirmasi siapa pun (atau staf yang akunnya sudah dihapus)
 * memang tidak punya email untuk ditampilkan.
 */
export function getStaffNameFromEmail(email: string | null | undefined): string {
  if (!email) return "—";
