 ...
 *
 * UPDATE v3 — RBAC (role Checker jadi read-only):
 * - Penguncian tiket sekarang berbasis STATUS ("confirmed"/"paid" =
 *   "Paid"), bukan lagi `verification_status` — begitu Paid, hanya Admin
 *   yang boleh mengedit; Kasir kehilangan haknya.
 * - Checker TIDAK PERNAH melihat tombol "+ Tambah Manual", "Edit", atau
 *   "Konfirmasi" di tab ini — semuanya disembunyikan total, bukan cuma
 *   dimatikan, karena role ini read-only.
 */
...
interface VisitorCardProps {
  ...
  canConfirm: boolean;
  /** True kalau tiket ini sudah Paid dan role yang login (Kasir) kehilangan haknya. */
  isLockedForRole: boolean;
  /** Checker read-only: tidak pernah melihat tombol Edit sama sekali. */
  canEdit: boolean;
          ...
          <div className="flex gap-2">
            {canEdit && (
              <button
                ...
                title={
                  isLockedForRole
                    ? "Tiket ini sudah Paid/Dikonfirmasi dan terkunci. Hubungi Admin."
                    : undefined
                }
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ role, sessionId }) => {
  ...
  const canEdit = role === "admin" || role === "kasir";
  const canCreateManual = role === "admin" || role === "kasir";
  /**
   * Tiket berstatus "Paid" (confirmed/paid) terkunci untuk KASIR saja —
   * eksklusif milik Admin. Aturan yang sama persis ditegakkan ulang di
   * backend `_assert_can_mutate`, layar ini hanya mencerminkannya lebih
   * awal supaya kasir tidak menabrak 403.
   */
  const isLockedFor = useCallback(
    (tx: TransactionEntry) => role === "kasir" && (tx.status === "confirmed" || tx.status === "paid"),
    [role]
  );
