 ...
 *
 * UPDATE v3 — Ticket Edit Access Validation (role Checker jadi read-only):
 * - PENGUNCIAN DATA sekarang berbasis STATUS tiket, bukan lagi
 *   `verification_status`: begitu status "confirmed"/"paid" ("Paid"),
 *   HANYA Admin yang boleh mengedit — Kasir kehilangan haknya. Modal
 *   tetap bisa dibuka (kasir berhak MELIHAT rincian), tapi seluruh input
 *   dimatikan dan alasannya ditulis di atas.
 * - Checker TIDAK PERNAH membuka modal ini sama sekali (tombol Edit
 *   disembunyikan total di layar pemanggil — lihat AdminDashboard.tsx &
 *   PaymentHistoryComponent.tsx), jadi cabang read-only untuk role ini
 *   di sini murni jaga-jaga tambahan.
 * - Hak HAPUS: hanya Admin. Kasir & Checker tidak pernah boleh menghapus.
 */
...
interface EditTransactionModalProps {
   ...
   * PER-TRANSAKSI (bergantung status tiketnya: pending vs Paid), bukan
   * sekali di level halaman seperti sebelumnya.
   */
  role: UserRole | null;
}
...
const EditTransactionModal: React.FC<EditTransactionModalProps> = ({
  ...
  /* =====================================================
     HAK AKSES PER-TRANSAKSI (Ticket Edit Access Validation)
     ...
     Aturan:
       - Admin  : selalu boleh edit & hapus.
       - Kasir  : boleh edit tiket yang BELUM "Paid" (pending); begitu
                  berstatus "confirmed"/"paid", haknya hilang — eksklusif
                  milik Admin. Tidak pernah boleh menghapus.
       - Checker: read-only total (seharusnya tidak pernah sampai
                  membuka modal ini — lihat catatan di header file).
  ===================================================== */
  const isPaid = transaction.status === "confirmed" || transaction.status === "paid";
  const isLocked = role === "kasir" && isPaid;
  const isReadOnlyRole = role !== "admin" && role !== "kasir"; // Checker & role tak dikenal.
  const readOnly = isSaving || isLocked || isReadOnlyRole;
  const canDelete = role === "admin";

  const lockNotice = isLocked
    ? "Tiket ini sudah berstatus Paid/Dikonfirmasi sehingga terkunci untuk Kasir. Hubungi Admin bila memang perlu diubah."
    : isReadOnlyRole
    ? "Role Checker bersifat read-only dan tidak dapat mengubah transaksi."
    : null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 animate-in fade-in duration-200">
      <div className="bg-[#fcfcfc] rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[95vh] border border-gray-200">
        ...
        <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
          ...
          {transaction.confirmed_by_email && (
            <div className="mb-6 flex items-center gap-2 flex-wrap">
              <span className="text-[11px] font-extrabold text-gray-500 uppercase tracking-wider">Dikonfirmasi Oleh:</span>
              <span className="text-[11px] font-bold px-2 py-1 rounded-full border bg-gray-100 text-gray-700 border-gray-300">
                {getStaffNameFromEmail(transaction.confirmed_by_email)}
              </span>
            </div>
          )}
