 ...
 * ----------------------------------------------------
 * Halaman riwayat transaksi. Diselaraskan dengan skema backend baru:
 * customer_name, ticket_code, ticket_name_snapshot per item, session_id.
...
const PaymentHistoryPage: React.FC = () => {
  ...
  const handleSessionSelect = (sessionName: string, start: string, end: string, targetDayOfWeek?: number) => {
  ...
  const loadCurrentUser = useCallback(async () => {
    try {
      const user = await apiGet<UserStaff>("/users/me");
      setCurrentUser(user);
    } catch {
      // Non-blocking: kalau gagal, tombol Edit/Hapus disembunyikan secara default (aman).
    }
  }, []);
  ...
  const loadTransactions = useCallback(async () => {
    try {
      setIsLoading(true);
      setError(null);
      let path = "/transactions/all";
      if (statusFilter !== "all") path += `?status=${statusFilter}`;
      const data = await apiGet<TransactionEntry[]>(path);
      setTransactions(data);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "Gagal mengambil data riwayat transaksi.");
    } finally {
      setIsLoading(false);
    }
  }, [statusFilter]);
  ...
  const filteredTransactions = useMemo(() => {
    return transactions.filter((tx) => {
      ...
      if (dateFilter) {
        ...
        const formattedTxDate = `${txDate.getFullYear()}-${String(txDate.getMonth() + 1).padStart(2, "0")}-${String(
          txDate.getDate()
        ).padStart(2, "0")}`;
      ...
      if (startTimeStr && endTimeStr) {
        ...
        if (!isNaN(startH) && !isNaN(endH)) {
          ...
          if (txMins < actualStart || txMins > actualEnd) return false;
        }
      }
  ...
  const activeFilterLabel = useMemo(() => {
    ...
    if (startTimeStr && endTimeStr) parts.push(`Jam: ${startTimeStr}-${endTimeStr}`);
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      filteredTransactions.forEach((tx) => {
        ...
        let pmStr = PAYMENT_METHOD_LABEL[tx.payment_method] || tx.payment_method?.toUpperCase() || "-";
      ...
      const now = new Date();
      const dateSuffix = `${now.getFullYear()}${(now.getMonth() + 1).toString().padStart(2, "0")}${now
        .getDate()
        .toString()
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          <header className="flex flex-col border-b-2 border-gray-200 pb-5 gap-4 mt-2">
            ...
            <div className="w-full flex flex-col md:flex-row items-start md:items-center gap-3 pt-4 border-t border-gray-100">
              <span className="text-xs font-bold text-gray-500 uppercase tracking-widest shrink-0">Pilih Sesi Waktu:</span>

              <div className="flex flex-wrap items-center gap-2">
                <button
                  onClick={() => handleSessionSelect("minggu_pagi", "09:00", "10:45", 0)}
                  className={`px-3 py-1.5 text-xs font-bold rounded-lg border transition-all ${
                    activeSession === "minggu_pagi" ? "bg-black text-white border-black shadow-md" : "bg-white text-gray-600 border-gray-300 hover:border-gray-400 hover:bg-gray-50"
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-bold rounded-lg border transition-all ${
                    activeSession === "minggu_siang" ? "bg-black text-white border-black shadow-md" : "bg-white text-gray-600 border-gray-300 hover:border-gray-400 hover:bg-gray-50"
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-bold rounded-lg border transition-all ${
                    activeSession === "sabtu" ? "bg-black text-white border-black shadow-md" : "bg-white text-gray-600 border-gray-300 hover:border-gray-400 hover:bg-gray-50"
                  }`}
                >
                  Sabtu (13.30 - 16.30)
                </button>
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-bold rounded-lg border transition-all ${
                    activeSession === "all" ? "bg-orange-50 text-[#fb9418] border-orange-200 shadow-sm" : "bg-white text-gray-600 border-gray-300 hover:border-gray-400 hover:bg-gray-50"
                  }`}
                >
                  Semua Waktu
                </button>
