...
const TicketSelectionPage: React.FC = () => {
  ...
  const floorsData: FloorData[] = useMemo(() => {
    ...
    session.active_tickets.forEach((st) => {
      ...
      // --- Logika Penentuan Keterangan Usia ---
      let ageRangeText = "";
      if (sub.min_age !== undefined && sub.min_age !== null && sub.max_age !== undefined && sub.max_age !== null) {
        ageRangeText = ` (${sub.min_age}-${sub.max_age})`;
      } else if (sub.min_age !== undefined && sub.min_age !== null) {
        ageRangeText = ` (≥ ${sub.min_age})`;
      } else if (sub.max_age !== undefined && sub.max_age !== null) {
        ageRangeText = ` (≤ ${sub.max_age})`;
      }

      const variantName = `${resolveName(sub.name_i18n, language, sub.name)}${ageRangeText}`;
