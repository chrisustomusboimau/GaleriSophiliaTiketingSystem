 ...
 * UPDATE:
 * - Menampilkan nama grup (Lantai/Master Tiket) dan varian tiket 
 *   secara konsisten dengan QueueDisplay.tsx.
 * - Menampilkan label usia secara otomatis di bawah varian tiket.
 */
...
const CounterInput: React.FC<CounterInputProps> = ({ label, ageLabel, price, value, onChange }) => {
  ...
  return (
    <div className="mb-3 p-4 border border-gray-200 rounded-xl bg-white shadow-sm">
      <div className="flex justify-between items-start mb-3">
        <div>
          <label htmlFor={inputId} className="font-bold text-black cursor-pointer block text-sm sm:text-base">
            {label}
          </label>
          {ageLabel && (
        ...
        <span className="text-gray-600 font-medium text-sm sm:text-base">{formatCurrency(price)}</span>
      </div>
      ...
      {numericValue > 0 && (
        <div className="mt-3 text-xs sm:text-sm text-right text-gray-500 pt-2 border-t border-gray-100">
...
const VisitorForm: React.FC = () => {
  ...
  /* 
   * Pengelompokan & Resolusi Nama Tiket Sesuai Pola QueueDisplay.tsx 
   */
  const groupedTickets: TicketGroup[] = useMemo(() => {
    ...
    relevantTickets.forEach((st) => {
      ...
      // Menggunakan resolveName persis seperti resolveSnapshot pada QueueDisplay
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-md mx-auto text-black pb-8">
      ...
      {/* Rincian Tiket Dikelompokkan Per Lantai / Master Tiket (Persis QueueDisplay) */}
      <div className="mb-6 border border-gray-200 rounded-xl bg-[#fcfcfc] p-5 shadow-sm space-y-6">
        <p className="text-gray-500 font-bold uppercase text-xs sm:text-sm tracking-widest border-b border-gray-200 pb-2">
          {translations.ticketDetails?.[language] || "Rincian Tiket"}
        </p>

        {groupedTickets.map((group) => (
          <div key={group.masterId} className="space-y-3">
            {/* Header Nama Lantai / Master Tiket */}
            <div className="flex items-center gap-2">
              <span className="w-2.5 h-2.5 rounded-full bg-[#fb9418]"></span>
              <p className="text-black font-extrabold text-base">{group.masterName}</p>
            </div>
      ...
      <div className="mb-6 p-5 border border-gray-200 rounded-xl bg-[#fcfcfc] shadow-sm">
        ...
        <button type="button" onClick={handleAddCountry} className="mt-4 font-bold text-[#fb9418] hover:text-orange-600 transition-colors flex items-center gap-1.5 text-sm">
